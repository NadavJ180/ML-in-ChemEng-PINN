...
This is the project's research log: every notable bug, design change, and investigation,
documented here as it was discovered. It's split out of [`README.md`](README.md) (which stays a
short, practical "how to run this" reference) so this detailed narrative doesn't bury the setup
and usage instructions. Read this if you want the full "why" behind a design decision, are
tracing a specific number back to how it was measured, or are writing the paper's
Discussion/Limitations section — most of that content originates here, and these findings affect
how the results in the repo's plots should be read.
...
- **Figure 2** (`figure2_phs_histogram_and_roc.png`): histogram of log10(PHS) and its ROC curve, both
  restricted to `Score3_PHS_full` specifically, matching the singular "the PHS" framing. The
  fuller 3-baseline ROC comparison remains available separately in `evaluate_phs.py`'s own `roc_curves.png`
...
**IEEE double-column sizing** is used throughout this new script (`IEEE_PAGE_WIDTH = 7.16in`
spanning both columns, explicit 8pt figure font, 300 DPI) rather than matplotlib's on-screen
...
"replace, don't just add" principle: `phs_vs_epsilon.png` (PHS only, all perturbation types overlaid
in one panel) was a strict subset of what `scores_vs_epsilon.png` already shows (every score including
PHS, per perturbation type, across separate panels) — removed, including its one unique feature (a
...
`case_28` (both k=1) score **above tau at ε=0, i.e. on their own unperturbed clean fields** (PHS=6.99
